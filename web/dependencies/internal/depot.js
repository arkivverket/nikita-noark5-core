 ...
 *  2. When a successful depot occurs, the user is pushed to the correct html page
...
var depot = app.controller('DepotController',
    ['$scope', '$http', 'depotService',
        function ($scope, $http, depotService) {
            ...
            $scope.selectedDepotRole = "arkivar";
            $scope.selectedDepotRole = "saksbehandler";

            // Set values for dropdown in webpage
            $scope.depotOptions = depotOptions;

            // index.html, booleans to display cards
            $scope.showDepot = true;
            ...
            (async () => {
                try {
                    $scope.oidc = await depotService.getDepotHrefViaOIDC();
             ...
             * doDepot
             *
             * Perform the actual depot and redirect the logged in user to the correct page
             *
             */
            $scope.doDepot = function () {
                (async () => {
                    try {
                        $scope.tokenInfo = await depotService.doDepot($scope.oidc[DEPOT_ENDPOINT],
                        ...
                        console.log("Logging in. Token is " + JSON.stringify(
                            ...
                            ". Redirecting to page for " + $scope.selectedDepotRole);
                        if ($scope.selectedDepotRole === ROLE_RECORDS_MANAGER) {
                            changeLocation($scope, recordsManagerPage, true);
                        } else if ($scope.selectedDepotRole === ROLE_LEADER) {
                            changeLocation($scope, leaderPage, true);
                        } else if ($scope.selectedDepotRole === ROLE_CASE_HANDLER) {
                            changeLocation($scope, caseHandlerPage, true);
                        } else if ($scope.selectedDepotRole === ROLE_POST_MOTTAK) {
                            changeLocation($scope, postmottakPage, true);
                        } else if ($scope.selectedDepotRole === ROLE_QUALITY_CONTROL) {
