...
public class RecordHateoasController
    ...
    // Retrieve all CrossReference associated with a Record
    // GET [contextPath][api]/arkivstruktur/registrering/{systemId}/kryssreferanse
    // https://rel.arkivverket.no/noark5/v5/api/arkivstruktur/kryssreferanse/
    @Operation(summary = "Retrieves all CrossReference associated with a Record" +
            " identified by a systemId")
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = OK_VAL,
    ...
    public ResponseEntity<CrossReferenceHateoas>
    findAllCrossReferenceAssociatedWithRecord(
            @Parameter(name = SYSTEM_ID,
                    description = "systemID of the Record to retrieve " +
                            "CrossReferences for",
                    required = true)
        ...
        return ResponseEntity.status(OK)
                .body(recordService
                        .findCrossReferenceAssociatedWithRecord(systemID));
    }
