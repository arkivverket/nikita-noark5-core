...
to Noark 5 version 4 (Noark 5v4), while the actual Noark standard is working a Noark 5 version 5.
...
holy-grail when it comes to API development. The Noark 5v4 interface standard is a Hateoas
level 3 API.
...
[source,bash]
----
$ curl --user admin:password 'http://localhost:8092/noark5v4/' -i -X GET \
...
$ curl -H Authorization: Basic bmlraXRhLWNsaWVudDpzZWNyZXQ=  \
    -H 'Accept:application/vnd.noark5-v4+json' -i -X GET 'http://localhost:8092/noark5v4/'
----
...
$   curl -i -X POST -d username=admin -d password=password -c /tmp/cookies.txt \
      -H 'Accept:application/vnd.noark5-v4+json' http://localhost:8092/noark5v4/login
----
...
$ curl -v -H 'Authorization: Basic bmlraXRhLWNsaWVudDpzZWNyZXQ=' \
  -X POST  'http://localhost:8092/noark5v4/oauth/token?grant_type=password&client_id=nikita-client&username=admin&password=password'
----
